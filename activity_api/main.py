...
def _find_logs_dir() -> Optional[Path]:
    """
    Find the logs directory - check both possible locations.
    chat_api writes to chat_api/logs/, activity_api originally expected project_root/logs/
    """
    project_root = Path(__file__).parent.parent
    logs_dirs = [
        project_root / "chat_api" / "logs",  # Where chat_api actually writes
        project_root / "logs",                # Original expected location
    ]
    
    for candidate in logs_dirs:
        if candidate.exists() and any(candidate.glob("activity_*.json")):
            return candidate
    return None
